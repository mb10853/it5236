    ...
    try:
        ...
        if gd_registrationcode == None or gd_registrationcode == "":
            ...
            with conn.cursor() as cur:
                ...
                cur.execute(sql, (gd_sessionid, gd_userid, gd_registrationcode,))
                conn.commit()
                
            # Read results
            with conn.cursor() as cur:
                sql = "SELECT usersessionid FROM usersessions WHERE userid=%s"
            ...
            elif result['usersessionid'] != gd_sessionid:
                raise Exception('ERROR: Session could not be created')
            else:
