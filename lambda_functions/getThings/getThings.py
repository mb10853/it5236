...
def lambda_handler(event, context):
    ...
    gd_platform = event['gameplatform']
    ...
    try:
        ...
        if gd_userid == None or gd_userid == "":
            ...
            with conn.cursor() as cur:
                sql = "SELECT gameid, thingname, thingcover, thingsummary, convert_tz(things.thingcreated,@@session.time_zone,'America/New_York') as thingcreated FROM things LEFT JOIN users ON things.thinguserid = users.userid WHERE thingregistrationcode = %s AND gameplatform = %s AND userid = %s ORDER BY things.thingcreated ASC"
                cursor = cur.execute(sql, (gd_registrationcode, gd_platform, gd_userid,))
                result = cur.fetchall()
            conn.commit()
